...
func Test_loadConfig(t *testing.T) {
	...
	afero.WriteFile(yamlFS, "test.yaml", yamlFileContent, 0644)
	afero.WriteFile(yamlFS, "test.yml", ymlFileContent, 0644)
	...
	afero.WriteFile(ymlFS, "test.yml", ymlFileContent, 0644)
	...
	afero.WriteFile(jsonFS, "test.json", jsonFileContent, 0644)
	...
	tests := []struct {
		...
		},
		{
			"test.json",
			args{jsonFS},
			yamlToCloudBuild(jsonFileContent),
			false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := loadConfig(tt.args.AppFs, tt.name)
